...
"""
    b0model(fmap, xwater, echotime; kwargs...)
...
function b0model(
    ...
    xfat::AbstractArray{<:Number,D} = zeros(Tx, size(fmap)),
    ...
    Base.require_one_based_indexing(df, echotime, fmap, relamp, smap, xfat, xwater)
    ...
    # check dimensions
    dims == size(xwater) || throw("bad xwater size $(size(xwater)) vs dims=$dims")
    dims == size(xfat) || throw("bad xfat size $(size(xfat)) vs dims=$dims")
    (dims..., nc) == size(smap) ||
